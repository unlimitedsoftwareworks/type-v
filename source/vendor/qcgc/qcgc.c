...
struct qcgc_shadowstack _qcgc_shadowstack;
struct qcgc_bump_allocator _qcgc_bump_allocator;
#if LOG_ALLOCATOR_SWITCH
size_t qcgc_allocations;
#endif
qcgc_state_t qcgc_state;
...
static qcgc_push_roots_cb_t root_callback = NULL;
static void* root_callback_context = NULL;
void qcgc_set_root_callback(qcgc_push_roots_cb_t cb, void* context) {
    root_callback = cb;
    root_callback_context = context;
}


void qcgc_collect(void) {
    if (root_callback) {
        // Invoke the callback to push roots dynamically
        root_callback(root_callback_context);
    }
    ...
    // remove all roots
    qcgc_pop_root(_qcgc_shadowstack.top - _qcgc_shadowstack.base);
